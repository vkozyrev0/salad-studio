...
| `comfyui-api` **wedges on `/prompt`** while still answering `/ready` 200 | Observed after ~8 sequential renders: Comfy logs nothing for new requests and the client times out. | Reallocate the instance. Keep batches modest; never trust `/ready` alone. |
...
9. **Does the `comfyui-api` wedge depend on batch size?** Observed after ~8
